...
Нашей реализации `min` и `max` не нужно заботиться об этом, все что нужно сделать это передать блок который делает сравнение и возвращает наибольший или наименьший элемент, всю остальную работу делает `reduce`. Мощно, не так ли?
